...
(function(){
	...
	function _stxLibrary_js(_stxThirdParty,_exports) {
		...
		STX.Drawing.annotation.prototype.edit=function(context){
			...
			STX.safeClickTouch(this.stx.controls.annotationSave, saveAnnotation(this));
			STX.safeClickTouch(this.stx.controls.annotationCancel, cancelAnnotation(this));
		 ...
		 * @constructor
		 */
		STX.QuoteFeed.Xignite=function(token,params){
			if(!params) params={};
			this.token=token;
		...
		STX.QuoteFeed.Xignite.prototype.isRTEquity=function(symbol){
			return false;
		};
		...
		STX.QuoteFeed.Xignite.prototype.fetch=function(params, cb){
			...
			var symbol=this.symbology(params.symbol);
			var isRTEquity=this.isRTEquity(symbol);
			var isBats=!isRTEquity && !params.noBats && this.isBats(symbol);
				...
				if(!params.update && isDaily){
					...
					if(isDaily){
						if(isMutual){
							...
							api=STX.clone(STX.QuoteFeed.Xignite.Templates.SuperQuoteEquity);
							if(params.extended) {
								api.results.extCls="ExtendedHoursPrice";
								api.fields+=",ExtendedHoursPrice";
							}
						}else if(isRTEquity){
							api=STX.clone(STX.QuoteFeed.Xignite.Templates.RealTimeEquity);
							if(params.extended) {
								api.results.extCls="ExtendedHoursPrice";
								api.fields+=",ExtendedHoursPrice";
							}
						...
						if(isMutual){
							...
							api=STX.clone(STX.QuoteFeed.Xignite.Templates.SuperIntradayEquity);
							if(!params.loadMore){
								api.results.timing="Timing";
								api.fields+=",Timing";
							}
						}else if(isRTEquity){
							api=STX.clone(STX.QuoteFeed.Xignite.Templates.IntradayRTEquity);
							realTimeQuote=true;
						}else{
							api=STX.clone(STX.QuoteFeed.Xignite.Templates.IntradayEquity);
						}
					...
					function processData(quotes, params){
						...
						for(i=1;i<arr.length;i++){
							...
							if(api.results.timing) {
								...
								else if(timing=="Historical") eodQuote=true;
							}
		...
		STX.QuoteFeed.Xignite.Templates={
			...
			SuperQuoteEquity: {
				...
				method:	"GetQuote",
				statics:"IdentifierType=Symbol",
				fields:	"DateTime,Volume,Open,High,Low,Last",
				results:{
					date:	"DateTime",
					time:	null,  //not needed for daily update
					open:	"Open",
		...
		*/
		 ...
		 * This visualization will only work if the `extended` and `marketSessions` parameters in the {@link STXChart#layout} object are set, 
		 ...
		 * See {@link STX.Market} for details on how to define extended (non-default) hours.
		 * 
		 ...
		 * 
		 * @param {STXChart} stx The chart object
		 * @param {array} sessions Names of the sessions to visualize. The names must match the session names declared in {@link STX.Market}.
		 * @constructor
		 * @name  STX.ExtendedHours
		 * @example
		 ...
		 * @since  06-2016-02
		 */
		STX.ExtendedHours=function(stx,sessions){
			this.stx=stx;
			var styles={};
			...
			stx.append("initializeDisplay", function(){
				...
				if(styles.session){
					...
					var noDashes=STX.isTransparent(styles.divider.backgroundColor);
